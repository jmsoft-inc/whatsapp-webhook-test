...
async function testRomslompProcessing() {
  try {
    ...
    console.log(`📄 Is Professional Invoice: ${isProfInvoice ? '✅ YES' : '❌ NO'}`);
    ...
    const result = createProfessionalInvoiceResponse(extractedText, invoiceNumber);
    ...
    // Calculate accuracy
    const fields = [
      { name: 'company_name', expected: expected.company_name, actual: result.company_name },
      { name: 'supplier_name', expected: expected.supplier_name, actual: result.supplier_name },
      { name: 'supplier_address', expected: expected.supplier_address, actual: result.supplier_address },
      { name: 'supplier_phone', expected: expected.supplier_phone, actual: result.supplier_phone },
      { name: 'supplier_email', expected: expected.supplier_email, actual: result.supplier_email },
      { name: 'supplier_website', expected: expected.supplier_website, actual: result.supplier_website },
      { name: 'supplier_btw', expected: expected.supplier_btw, actual: result.supplier_btw },
      { name: 'supplier_kvk', expected: expected.supplier_kvk, actual: result.supplier_kvk },
      { name: 'supplier_iban', expected: expected.supplier_iban, actual: result.supplier_iban },
      { name: 'supplier_bic', expected: expected.supplier_bic, actual: result.supplier_bic },
      { name: 'invoice_date', expected: expected.invoice_date, actual: result.invoice_date },
      { name: 'due_date', expected: expected.due_date, actual: result.due_date },
      { name: 'invoice_reference', expected: expected.invoice_reference, actual: result.invoice_reference },
      { name: 'customer_name', expected: expected.customer_name, actual: result.customer_name },
      { name: 'customer_address', expected: expected.customer_address, actual: result.customer_address },
      { name: 'subtotal_excl_btw', expected: expected.subtotal_excl_btw, actual: result.subtotal_excl_btw },
      { name: 'btw_amount', expected: expected.btw_amount, actual: result.btw_amount },
      { name: 'btw_percentage', expected: expected.btw_percentage, actual: result.btw_percentage },
      { name: 'total_amount', expected: expected.total_amount, actual: result.total_amount },
      { name: 'currency', expected: expected.currency, actual: result.currency },
      { name: 'payment_terms', expected: expected.payment_terms, actual: result.payment_terms },
      { name: 'items_count', expected: expected.items_count, actual: result.items.length }
    ];
