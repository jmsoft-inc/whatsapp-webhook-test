...
console.log(`GOOGLE_SHEETS_SPREADSHEET_ID: ${spreadsheetId ? '✅ Set' : '❌ Missing'}`);
console.log(`GOOGLE_SHEETS_CREDENTIALS: ${credentialsJson ? '✅ Set' : '❌ Missing'}\n`);

if (!spreadsheetId || !credentialsJson) {
  console.log('❌ Missing required environment variables!');
  console.log('Please set both GOOGLE_SHEETS_SPREADSHEET_ID and GOOGLE_SHEETS_CREDENTIALS');
  process.exit(1);
}
...
async function testConnection() {
  try {
    ...
    console.log(`📝 First row has ${readResponse.data.values ? readResponse.data.values[0].length : 0} columns`);
    ...
    if (error.code === 403) {
      console.log('\n🔧 Possible solutions:');
      console.log('1. Make sure the service account email has access to the spreadsheet');
      console.log('2. Check that Google Sheets API is enabled in Google Cloud Console');
